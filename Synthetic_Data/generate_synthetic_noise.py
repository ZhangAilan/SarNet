...
def gen_noise_corr_dem():
    ...
    noisy_dem1   = dem_scaled1*noise #将噪声应用到dem上，生成噪声干扰的dem
    noisy_dem2   = dem_scaled2*noise1
    noisy_dem3   = dem_scaled3*noise
    return noisy_dem1,noisy_dem2,noisy_dem3  #返回三个噪声干扰的dem
...
def set_noise_params():
    ...
    while get_dem:
        f1 = np.random.randint(0,len(files))  #随机选择一个dem文件
    ...
    ###############################################################################
    #Make Noise
    noise = make_corr_noise(nparams)  #相关噪声
...
def corr_noise(covd,numdatas):  #根据协方差矩阵生成大气相关噪声
    npoints = len(covd)
