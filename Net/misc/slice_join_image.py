...
class Tile(object):
    ...
    def __init__(self, image, number, position, coords, filename=None, score=0, cam=None):
        self.image = image
        self.number = number
    ...
    @property  # read-only,无需调用方法
    def row(self):
        return self.position[0] #返回行索引

    @property
    def column(self):
        return self.position[1] #返回列索引

    @property
    def basename(self):
        """Strip path and extension. Return base filename."""
        return get_basename(self.filename) #从给定的文件名中去除路径和扩展名，返回基本文件名
